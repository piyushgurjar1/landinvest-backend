...
URL_ENRICH_API = "https://test-9kmq.onrender.com/enrich"
URL_ENRICH_TIMEOUT = httpx.Timeout(
    connect=20.0,
    read=180.0,
    write=20.0,
    pool=20.0,
...
def to_int_zero(v: Any) -> int:
    return to_int(v) or 0
...
def maybe_percent(num: Optional[float]) -> Optional[str]:
    if num is None:
        return None
    return f"{num:.1f}%"
...
def parse_date(s: Optional[str]) -> Optional[date]:
    if not s:
        return None
    s = s.strip()
    fmts = ["%Y-%m-%d", "%m/%d/%Y", "%m-%d-%Y", "%b %d, %Y", "%B %d, %Y", "%Y/%m/%d"]
    for fmt in fmts:
        try:
...
def median_int(values: list[float | int]) -> int:
...
def avg_int(values: list[float | int]) -> int:
    values = [float(v) for v in values if v is not None]
    if not values:
        return 0
...
def relative_diff_pct(a: float, b: float) -> float:
    if a == 0 and b == 0:
        return 0.0
...
def build_maps_links(gps: Optional[str]) -> tuple[Optional[str], Optional[str], Optional[str]]:
    gps = clean_str(gps)
...
def build_baseline_context(parcelinfo: Any) -> str:
    if not parcelinfo:
        return "- None"
    lines = []
    fields = [("address", "Address")]
    for attr, label in fields:
        val = getattr(parcelinfo, attr, None)
        if val not in [None, ""]:
            lines.append(f"- {label}: {val}")
    lat = getattr(parcelinfo, "latitude", None)
    lng = getattr(parcelinfo, "longitude", None)
    if lat and lng:
        lines.append(f"- GPS: {lat}, {lng}")
    return "\n".join(lines) if lines else "- None"


def utility_status_to_bool(status: str) -> Optional[bool]:
    if status == "confirmed":
        return True
...
def yes_no_unknown_to_bool(status: str) -> Optional[bool]:
    if status == "yes":
        return True
    if status == "no":
        return False
...
def merge_all_stages(
    ...
    return {
        ...
        "sqft": identity.sqft or int(round((identity.acreage or 0) * 43560)) if identity.acreage else 0,
        ...
        "buildable": yes_no_unknown_to_bool(zoning.buildable),
        "residential_allowed": yes_no_unknown_to_bool(zoning.residential_allowed),
        "mobile_homes_allowed": yes_no_unknown_to_bool(zoning.mobile_homes_allowed),
        "rv_allowed": yes_no_unknown_to_bool(zoning.rv_allowed),
        "tiny_homes_allowed": yes_no_unknown_to_bool(zoning.tiny_homes_allowed),
        "camping_allowed": yes_no_unknown_to_bool(zoning.camping_allowed),
        "off_grid_allowed": yes_no_unknown_to_bool(zoning.off_grid_allowed),
        "commercial_allowed": yes_no_unknown_to_bool(zoning.commercial_allowed),
        "agricultural_allowed": yes_no_unknown_to_bool(zoning.agricultural_allowed),
        "hoa_present": yes_no_unknown_to_bool(zoning.hoa_present),
        ...
        "electricity_available": utility_status_to_bool(utilities.electricity.status),
        "water_available": utility_status_to_bool(utilities.water.status),
        "sewer_available": utility_status_to_bool(utilities.sewer.status),
        "gas_available": utility_status_to_bool(utilities.gas.status),
        "well_required": yes_no_unknown_to_bool(utilities.well_required.status),
        "septic_required": yes_no_unknown_to_bool(utilities.septic_required.status),
        "utility_at_street": yes_no_unknown_to_bool(utilities.utility_at_street.status),
        ...
        "power_lines_visible": yes_no_unknown_to_bool(environment.power_lines_visible),
        ...
        "sources_used_stage1": dedupe_keep_order(
...
def dedupe_comps(comps: list[RawSoldComp]) -> list[RawSoldComp]:
    seen = set()
    out = []
    for c in comps:
        ...
        key = (clean_str(c.apn), c.sold_price, round(c.acreage, 4), clean_str(c.sold_date), clean_str(c.source_url))
        if key in seen:
            continue
        seen.add(key)
        out.append(c)
...
def dedupe_listings(rows: list[RawActiveListing]) -> list[RawActiveListing]:
    seen = set()
    out = []
    for r in rows:
        ...
        key = (clean_str(r.apn), r.listing_price, round(r.acreage, 4), clean_str(r.source_url))
        if key in seen:
            continue
        seen.add(key)
        out.append(r)
...
def filter_outliers_by_median_band(values: list[int], lower_mult: float, upper_mult: float) -> tuple[int, float, float]:
    med = median_int(values)
    return med, med * lower_mult, med * upper_mult


def clean_market_data(stage2_raw: Stage2Raw) -> dict[str, Any]:
    raw_comps = dedupe_comps(stage2_raw.raw_sold_comps)
    raw_listings = dedupe_listings(stage2_raw.raw_active_listings)
    ...
    if base_comps:
        ...
        med, lower, upper = filter_outliers_by_median_band(vals, 0.40, 2.5)
        for c in base_comps:
        ...
        if len(strict_clean) < 3:
            med, lower, upper = filter_outliers_by_median_band(vals, 0.30, 3.5)
            strict_clean = []
    ...
    median_sold_ppa = median_int(sold_ppa)
    avg_sold_ppa = avg_int(sold_ppa)
    avg_active_ppa = avg_int(active_ppa)
    ...
    sold_to_active = round(sales12 / inventory, 2) if inventory else None
    median_dom = median_int(doms) if doms else None
    sales_velocity = f"{sales12 / 12:.1f} sales/month"
    ...
    zip_turnover_6 = sales6 / inventory * 100.0 if inventory else None
    zip_turnover_12 = sales12 / inventory * 100.0 if inventory else None

    diff_pct = relative_diff_pct(median_sold_ppa, avg_sold_ppa) if median_sold_ppa and avg_sold_ppa else 100.0
    ...
    return {
        ...
        "market_demand": {
            "land_sales_last_6_months": sales6,
            "land_sales_last_12_months": sales12,
            ...
            "zip_turnover_rate_6mo": maybe_percent(zip_turnover_6),
            "zip_turnover_rate_12mo": maybe_percent(zip_turnover_12),
...
def _url_enrich_key(item: dict[str, Any]) -> tuple[str, str]:
    return (
...
async def _call_remote_url_enricher(payload: dict[str, Any]) -> list[dict[str, Any]]:
    last_error = None

    for attempt in range(1, 4):
        try:
            async with httpx.AsyncClient(
                timeout=URL_ENRICH_TIMEOUT,
                follow_redirects=True,
            ) as client:
                res = await client.post(URL_ENRICH_API, json=payload)

            logger.info(
                "Remote URL enrich attempt=%s status_code=%s",
                attempt,
                res.status_code,
            )
            ...
            if not isinstance(results, list):
                raise ValueError("Remote enrich response missing 'results' list")

            logger.info(
                "Remote URL enrich attempt=%s results_count=%s",
                attempt,
                len(results),
            )
            ...
            last_error = e
            logger.warning("Remote URL enrich failed attempt=%s error=%s", attempt, e)
            if attempt < 3:
                await asyncio.sleep((attempt * 6) + random.uniform(1.0, 3.0))

        except Exception as e:
            last_error = e
            logger.exception("Unexpected remote URL enrich error attempt=%s error=%s", attempt, e)
            if attempt < 3:
                await asyncio.sleep((attempt * 6) + random.uniform(1.0, 3.0))

    logger.error("Remote URL enrich failed after retries error=%s", last_error)
    return []
...
async def _enrich_source_urls(stage2b: dict[str, Any]) -> None:
    ...
    payload = {
        "clean_sold_comps": [
            {"address": item.get("address"), "source": item.get("source")}
    ...
    logger.info(
        "Calling remote URL enrich API sold_count=%s active_count=%s",
        len(payload["clean_sold_comps"]),
        len(payload["clean_active_listings"]),
    )

    results = await _call_remote_url_enricher(payload)

    if not results:
        for item in all_items:
            item.setdefault("source_url", None)
        logger.warning("Remote URL enrich returned no results")
        return

    buckets: dict[tuple[str, str], list[str | None]] = defaultdict(list)

    for result in results:
        key = _url_enrich_key(result)
        buckets[key].append(result.get("url"))

    for item in all_items:
        key = _url_enrich_key(item)
        url = None

        if buckets.get(key):
            url = buckets[key].pop(0)

        item["source_url"] = url

        if url:
            logger.info("Enriched URL: %s -> %s", item.get("address"), url)
        else:
            logger.debug(
                "No remote URL found for address=%s source=%s",
                item.get("address"),
                item.get("source"),
            )
...
def liens_found(liens_beyond_tax: Optional[str]) -> bool:
    s = norm_text(liens_beyond_tax)
    if not s:
        return False
    return s not in {"none identified", "none", "no liens found", "unknown"}


def protected_or_env(core: dict[str, Any]) -> bool:
    a = norm_text(core.get("protected_land_status"))
    b = norm_text(core.get("environmental_restrictions"))
    bad = {"", "none", "none identified", "n/a", "na", "unknown", "not found", "no known restrictions"}
    return a not in bad or b not in bad


def risk_score(stage1: dict[str, Any], stage2b: dict[str, Any]) -> tuple[int, list[str], str]:
    score = 0
    factors = []
    ...
    fire = norm_text(stage1.get("fire_risk"))
    ...
    if protected_or_env(stage1):
        score += 15
        factors.append("Protected land/environmental restriction +15")

    util_count = sum(1 for k in ["electricity_available", "water_available", "sewer_available", "gas_available"] if stage1.get(k) is True)
    if util_count == 0:
        score += 10
        factors.append("No confirmed public utilities +10")

    if liens_found(stage1.get("liens_beyond_tax")):
        score += 10
        factors.append("Liens/encumbrances found +10")

    landslide = norm_text(stage1.get("landslide_risk"))
...
def deal_score(stage1: dict[str, Any], stage2b: dict[str, Any], profit_margin_pct_num: float) -> tuple[int, list[str], str]:
    factors = []
    total = 0

    road = norm_text(stage1.get("road_type"))
    access = norm_text(stage1.get("legal_access_status") or stage1.get("road_condition") or "")

    if "paved" in road and ("legal" in access or "confirmed" in access or "good" in access):
        pts, note = 5, "Location 5/5 Paved road and legal access confirmed"
    elif "gravel" in road and ("legal" in access or "confirmed" in access or "good" in access):
        pts, note = 4, "Location 4/5 Gravel road and legal access confirmed"
    elif "dirt" in road or "unclear" in access or "unknown" in access or "no road" in access:
        pts, note = 2, "Location 2/5 Dirt road or access unclear"
    else:
        pts, note = 1, "Location 1/5 Weak access signal"
    ...
    risk_score_num, _, _ = risk_score(stage1, stage2b)
...
def risk_tier(risk_score_num: int) -> str:
...
def build_red_flags(stage1: dict[str, Any], stage2b: dict[str, Any], max_bid_ceiling: int) -> list[str]:
    flags = []

    if stage1.get("landlocked") is True:
        flags.append("Parcel appears landlocked")

    road = norm_text(stage1.get("road_type") or "")
    ...
    if "extreme" in norm_text(stage1.get("fire_risk")):
...
def next_learning_step(stage1: dict[str, Any], red_flags: list[str]) -> str:
...
def compute_mlf(market_classification: str) -> float:
    return {
        "Fast": 0.90,
        "Normal": 0.85,
        "Slow": 0.80,
...
def compute_cf(stage1: dict[str, Any]) -> tuple[float, list[str]]:
    cf = 0.12
    adjustments = []
...
FL_RISK_BUFFER = 0.08
FL_AUCTION_FEE = 0.05
FL_TARGET_MARGIN = 0.40
FL_FIXED_COSTS = 0
FL_LF_DEFAULT = 0.92


def is_florida(state: str | None) -> bool:
    return norm_text(state or "") in {"fl", "florida"}


def compute_florida_bid(
    mmv: float,
    stage1: dict[str, Any],
    stage2b: dict[str, Any],
    fixed_costs: float = FL_FIXED_COSTS,
    auction_price: float | None = None,
) -> dict[str, Any]:
    lf = FL_LF_DEFAULT
    exit_price = mmv * lf
    net_mult = 1.0 - FL_SELL_COST - FL_CARRY_COST - FL_RISK_BUFFER
    denom = 1.0 + FL_TARGET_MARGIN + FL_AUCTION_FEE
    ...
    bid_formula_note = (
        f"FL MAX BID = MMV*LF*NETMULT - FIXED / DENOM | "
        f"MMV {money(int(mmv))} | LF {lf} | EXIT {money(int(exit_price))} | "
        f"NETMULT {net_mult:.2f} | DENOM {denom:.2f} | MAX BID {money(max_bid)}"
    )
    resale_formula_note = (
        f"FL Resale: Low 88% of MMV {money(conservative_resale)} | "
        f"Mid 92% of MMV {money(suggested_resale_price)} | "
        f"High 96% of MMV {money(aggressive_resale)}"
    )
    ...
    return {
        ...
        "florida_sell_cost": FL_SELL_COST,
        "florida_carry_cost": FL_CARRY_COST,
        "florida_risk_buffer": FL_RISK_BUFFER,
        "florida_auction_fee": FL_AUCTION_FEE,
        "florida_target_margin": FL_TARGET_MARGIN,
...
def build_report(stage1: dict[str, Any], stage2b: dict[str, Any]) -> dict[str, Any]:
    acreage = safe_float(stage1.get("acreage"), 0.0)
    median_ppa = to_int_zero(stage2b.get("median_sold_price_per_acre"))
    avg_sold_ppa = to_int(stage2b.get("avg_sold_price_per_acre"))
    avg_active_ppa = to_int(stage2b.get("avg_active_price_per_acre"))
    ...
    diff_pct = relative_diff_pct(median_ppa, avg_sold_ppa or 0) if median_ppa and avg_sold_ppa else None

    valuation_notes = f"Median sold price/acre {money(median_ppa)}, average sold price/acre {money(avg_sold_ppa)}, using {clean_comp_count} clean comps, confidence {data_confidence}."
    ...
    mlf = compute_mlf(market_classification)
    cf, cf_adjustments = compute_cf(stage1)
    pm = 0.40
    ...
    bid_formula_note = (
        f"MAX BID = MMV * MLF * (1 - PM - CF) | "
        f"MMV {money(int(mmv))} | MLF {mlf_pct}% {market_classification} | "
        f"PM 40% | {cf_note} | RESULT {money(max_bid_ceiling)}"
    )
    resale_formula_note = (
        f"Low {mlf_pct - 5}% of MMV | Mid {mlf_pct}% of MMV | High {mlf_pct + 3}% of MMV | "
        f"MLF driven by {market_classification} market"
    )

    risk_score_num, risk_factors_applied, risk_explanation = risk_score(stage1, stage2b)
    deal_score_num, scoring_factors, deal_explanation = deal_score(stage1, stage2b, profit_margin_num)
    verdict_val = verdict(deal_score_num)

    market = stage2b["market_demand"]
    resale_timeline = market.get("estimated_dom_range")
    red_flags = build_red_flags(stage1, stage2b, max_bid_ceiling)
    next_step = next_learning_step(stage1, red_flags)

    state_val = stage1.get("state")
    florida_bid_data = None
    if is_florida(state_val):
        florida_bid_data = compute_florida_bid(mmv, stage1, stage2b)
        if florida_bid_data["florida_hard_stop"]:
            red_flags.insert(
                0,
                f"FL HARD STOP: Auction/MMV ratio {florida_bid_data['florida_ratio']:.2f} exceeds 0.85 threshold. DO NOT BID.",
            )
            verdict_val = "NO BID"
        elif florida_bid_data["florida_ratio_tier"] == "CAUTION":
            red_flags.insert(
                0,
                f"FL CAUTION: Auction/MMV ratio {florida_bid_data['florida_ratio']:.2f} is in the 0.75-0.85 warning zone.",
            )
    ...
    road_type = norm_text(stage1.get("road_type") or "")
    ...
    bid_recommendation = (
        f"Recommended Max Bid {money(max_bid_ceiling)} | "
        f"Resale {money(suggested_resale_price)} | "
        f"Profit {money(expected_profit)} | {verdict_val}"
    )

    summary = (
        f"This {acreage:.2f}-acre parcel in {stage1.get('county')}, {stage1.get('state')} "
        f"has a mid estimated market value of {money(mid_estimated_value)} based on median sold "
        f"price per acre. The local market is classified as {market.get('market_classification') or 'Unknown'}. "
        f"Using MLF {mlf_pct}% and CF {cf_pct}%, the recommended bid ceiling is {money(max_bid_ceiling)}, "
        f"and the expected resale target is {money(suggested_resale_price)}."
    )

    sources_checked = dedupe_keep_order(
        stage1.get("sources_used_stage1", []) + stage2b.get("sources_used_stage2", [])
    )

    report = ParcelReport(
        home_page_summary={
            "parcel_size_acres": acreage or None,
            "parcel_size_sqft": stage1.get("sqft"),
        ...
        resale_price_range={
            ...
            "resale_formula_note": resale_formula_note,
            "recommended_tier_explanation": f"Suggested tier uses MLF {mlf_pct}% of MMV based on {market_classification} market conditions.",
        },
        auction_bid_ceiling={
            "risk_tier": risk_tier(risk_score_num),
            "mlf": f"{mlf_pct}%",
            "cf": f"{cf_pct}%",
        ...
        compliance_disclaimer=(
            ...
            "This is not financial, legal, or tax advice and is not a recommendation to buy, sell, or offer any amount on this or any other property."
        ),
    )
...
async def analyze_apn(
    ...
    baseline_context = build_baseline_context(parcelinfo)
    ...
    maps_link, sat_link, street_link = build_maps_links(stage1a.gps_coordinates)
    ...
    acreage_str = str(stage1a.acreage or getattr(parcelinfo, "lot_size", "") or "")

    stage1b, stage1c, stage1d, stage2raw = await asyncio.gather(
        generate_structured(
    ...
    stage2b = clean_market_data(stage2raw)

    try:
        await asyncio.wait_for(_enrich_source_urls(stage2b), timeout=240.0)
    except asyncio.TimeoutError:
        logger.warning("Remote URL enrichment timed out; using existing URLs only")

    final_report = build_report(stage1=stage1, stage2b=stage2b)
